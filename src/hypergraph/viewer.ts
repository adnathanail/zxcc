...
/** Provisional, and deliberately uniform: colouring blobs by spider type is
 *  still open. Translucent fill so overlapping blobs stay readable — every dot
 *  that isn't a boundary leg is shared by two of them. */
const BLOB_FILL = 'rgba(124, 179, 66, 0.18)'
const BLOB_STROKE = 'rgb(85, 139, 47)'
    ...
    return svg`
      <g data-hyperedge=${blob.id}>
        <path d=${blobOutline(blob, pos, scene.blobRadius)}
          fill=${BLOB_FILL} stroke=${BLOB_STROKE}
          stroke-width="2" stroke-linejoin="round" />
        ${
          anchor
            ? svg`<text x=${anchor.x} y=${anchor.y} text-anchor="middle" font-size="11px"
                font-family="monospace" fill=${BLOB_STROKE}
          ...
          ${scene.dots.map(
            dot => svg`
              <g data-wire=${dot.id} transform="translate(${dot.x},${dot.y})">
                <circle r=${scene.dotSize} fill="black" />
                ${
                  this.showLabels
